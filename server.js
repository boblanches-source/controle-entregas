...
app.post('/api/orders', async (req, res) => {
  try {
    const { customer_name, initial_payment_method, has_drink, paid_at_counter = false, needs_change = false, order_value, observations = '' } = req.body;
    ...
    const payload = {
      ...
      needs_change: Boolean(needs_change),
...
app.patch('/api/orders/:id', async (req, res) => {
  try {
    ...
    if (order.status !== STATUS.WAITING) {
      return res.status(400).json({ error: 'Somente pedidos aguardando podem ser editados.' });
    }
    ...
    await db('orders')
      ...
      .update({
        ...
        initial_payment_method,
        paid_at_counter: Boolean(paid_at_counter),
        needs_change: initial_payment_method === 'Dinheiro' && !Boolean(paid_at_counter) ? Boolean(needs_change) : false,
        has_drink: Boolean(has_drink),
        observations: String(observations || '').trim(),
        updated_at: db.fn.now(),
      });
...
app.patch('/api/orders/:id/finish', async (req, res) => {
  try {
    const { id } = req.params;
    const { final_payment_method, cash_received = null, cash_change = null } = req.body;
    ...
    if (!isValidPaymentMethod(final_payment_method) && !order.paid_at_counter) {
    ...
    await db('orders')
      ...
      .update({
        ...
        final_payment_method: order.paid_at_counter ? order.initial_payment_method : final_payment_method,
        cash_received: order.paid_at_counter ? null : cash_received,
        cash_change: order.paid_at_counter ? null : cash_change,
